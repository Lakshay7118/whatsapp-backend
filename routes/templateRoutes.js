...
// super_admin → all pending templates
// manager     → only their own pending templates
// =======================
router.get(
  "/templates/pending",
  protect,
  allowRoles("super_admin", "manager"), // ✅ added "manager"
  async (req, res) => {
    try {
      let filter = { approvalStatus: "pending_approval" };

      // ✅ Manager only sees their own pending templates
      if (req.user.role === "manager") {
        filter.createdBy = req.user.id;
      }
      // super_admin sees ALL pending templates
      ...
      res.json({ 
        success: true, 
        templates,
        canApprove: req.user.role === "super_admin", // ✅ tell frontend if approve/reject is allowed
      });
    } catch (error) {
